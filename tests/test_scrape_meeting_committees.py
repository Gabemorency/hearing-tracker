...
def test_pending_meetings_prioritizes_dates_nearest_today():
    # DomeWatch returns meetings oldest-first (real observed behavior) — a
    # meeting from months ago sits earlier in the list than one from today,
    # but should be scraped LAST, not first, since it's what's actually
    # visible on the calendar (today's) that matters.
    today = datetime(2026, 8, 3, tzinfo=timezone.utc)
    meetings = [
        {"id": "old",   "url": "http://x/old",   "startDate": "2026-05-08T19:00:00.000Z"},
        {"id": "today", "url": "http://x/today", "startDate": "2026-08-03T19:00:00.000Z"},
        {"id": "near",  "url": "http://x/near",  "startDate": "2026-08-01T19:00:00.000Z"},
    ]
    pending = smc.pending_meetings(meetings, {}, max_per_run=2, today=today)
    assert [m["id"] for m in pending] == ["today", "near"]


def test_pending_meetings_unparseable_date_sorts_last():
    today = datetime(2026, 8, 3, tzinfo=timezone.utc)
    meetings = [
        {"id": "no-date", "url": "http://x/1"},
        {"id": "today",   "url": "http://x/2", "startDate": "2026-08-03T19:00:00.000Z"},
    ]
    pending = smc.pending_meetings(meetings, {}, today=today)
    assert [m["id"] for m in pending] == ["today", "no-date"]
